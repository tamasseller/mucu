...
test("reg3", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("reg2", () => assert.deepEqual(disassemble({content: fromRaw(
    0x4008, // ands	r0, r1
    0x4051, // eors	r1, r2
    0x415a, // adcs	r2, r3
    0x41a3, // sbcs	r3, r4
...
test("shift", () => assert.deepEqual(disassemble({content: fromRaw(
    0x4088, // lsls	r0, r1
    0x40d1, // lsrs	r1, r2
    0x411a, // asrs	r2, r3
    0x00e3, // lsls	r3, r4, #3
...
test("cmp", () => assert.deepEqual(disassemble({content: fromRaw(
    0x4288, // cmp	r0, r1
    0x4592, // cmp	r10, r2
    0x4561, // cmp	r1, r12
    0x45e3, // cmp	r11, r12
...
test("loadstore", () => assert.deepEqual(disassemble({content: fromRaw(
    0x5088, // str   r0, [r1, r2]
    0x61a3, // str   r3, [r4, #24]
...
test("movs", () => assert.deepEqual(disassemble({content: fromRaw(
    0x25ea, // movs	r5, #234	@ 0xea
)}), 
`     movs r5, #234`))

test("spRel", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("special", () => assert.deepEqual(disassemble({content: fromRaw(
    0xde0c, // udf	#12
    0xdf17, // svc	23
    0xbe22, // bkpt	0x0022
)}), 
...
test("noArg", () => assert.deepEqual(disassemble({content: fromRaw(
    0xb662, // cpsie	i
    0xb672, // cpsid	i
    0xbf00, // nop
    0xbf10, // yield
...
test("hiReg", () => assert.deepEqual(disassemble({content: fromRaw(
    0x4408, // add	r0, r1
    0x44da, // add	r10, r11
    0x4611, // mov	r1, r2
    0x46e3, // mov	r11, r12
...
test("pushPop", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("mia", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("branch", () => assert.deepEqual(disassemble({content: fromRaw(
    0xd0fe, // beq.n	e <l>
    0xd1fd, // bne.n	e <l>
    0xd2fc, // bcs.n	e <l>
    0xd3fb, // bcc.n	e <l>
...
test("pcRel", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("stackFill", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("stackFillOffset", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("copyWords", () => assert.deepEqual(disassemble({content: fromRaw(
...
test("mixedWidth", () => assert.deepEqual(disassemble({content: fromRaw(
     0xf3bf, 0x8f4f,     //   dsb	sy
     0xa002,             //   add	r0, pc, #8	@ (adr r0, 10 <X>)
     0xf3bf, 0x8f6f,     //   isb	sy
     0x4670,             //   mov	r0, lr
     0xe7f8,             //   b.n	0 <a>
     0x46c0,             //   nop			@ (mov r8, r8)
     0xc0de, 0x1337,     //   .word	0x1337c0de
)}), 
`l0:  dsb 
     adr r0, L0
     isb 
